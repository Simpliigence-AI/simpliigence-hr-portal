...
import { useAccess, ROLES } from '@/lib/access';

export default function Sidebar() {
  const path = usePathname();
  const router = useRouter();
  const { loading, email, role, tabs } = useAccess();
  ...
  const displayName = email
    ? email.split('@')[0].split('.').map((p: string) => p.charAt(0).toUpperCase() + p.slice(1)).join(' ')
    : null;
  ...
  const avatarColour = email ? avatarColours[email.charCodeAt(0) % avatarColours.length] : 'bg-blue-500';

  const roleLabel = ROLES.find((r) => r.key === role)?.label ?? null;
  ...
  return (
    <aside className="w-60 min-h-screen bg-[#0f1e3d] text-white flex flex-col shrink-0">
      ...
      {email && (
        <div className="mx-3 mt-3 mb-1 rounded-xl bg-white/8 border border-white/10 px-3 py-2.5 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[11px] text-white/50 truncate">{roleLabel ?? email}</div>
          </div>
          <div className="w-2 h-2 rounded-full bg-green-400 shrink-0" title="Online" />
        </div>
      )}

      {/* Nav — sections come from the access matrix */}
      <nav className="flex-1 p-3 space-y-0.5 mt-1 overflow-y-auto">
        {loading && <div className="px-3 py-2 text-xs text-white/40">Loading menu…</div>}

        {!loading &&
          tabs.map((item, idx, arr) => {
            const { href, label, icon, section, key } = item;
        ...
        {!loading && tabs.length === 0 && (
          <div className="px-3 py-2 text-xs text-white/40">
            No tabs are enabled for your role. Ask an admin to update Access Control.
          </div>
        )}
      </nav>
      ...
      <div className="p-3 border-t border-white/10">
        ...
        <div className="text-xs text-white/30 text-center mt-2">HR Portal v1.3 · Aug 2026</div>
      </div>
    </aside>
  );
}
