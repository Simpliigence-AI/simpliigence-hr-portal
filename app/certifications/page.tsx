...
const STATUS_OPTS = ['Active','Retired','Maintenance Due']

function statusBadge(s: string | null) {
  if (!s) return null
  const cls = s === 'Active' ? 'bg-green-100 text-green-700' : s === 'Retired' ? 'bg-gray-100 text-gray-500' : 'bg-amber-100 text-amber-700'
                    ...
                    <div key={c.id} className="bg-white rounded-xl border border-gray-200 px-3 py-2.5 hover:shadow-md transition-shadow">
                      ...
                      <div className="flex items-center justify-between gap-1">
                        <span className="text-xs text-gray-400 truncate">
                          {groupBy === 'type' ? (c.dept || c.region || '—') : groupBy === 'employee' ? (c.issuer || '—') : c.cert_name}
                        </span>
